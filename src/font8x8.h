// font8x8.h -- an 8x8 bitmap font (public-domain "font8x8_basic" by D. Hepper).
// ============================================================================
//
// One glyph = 8 bytes, one byte per pixel-row (top row first). Within a row the
// bit order is LSB = LEFTMOST column (bit 0 is the left pixel, bit 7 the right).
// draw_char() matches this with `byte & (1 << col)`. Control chars 0x00-0x1F are
// blank; this covers ASCII through 0x7F.
#pragma once
#include <stdint.h>
...
// Return the 8-byte glyph for `ch`. Non-ASCII (>= 0x80) renders as '?'.

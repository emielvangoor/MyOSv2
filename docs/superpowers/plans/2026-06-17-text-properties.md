...
**Dual-build constraint (critical):** `src/rd_core.c` is compiled into **BOTH** the kernel (via `CSRC := $(wildcard src/*.c)`) **and** `lm.elf` (via `LM_CORE`), and `src/tests.c` (kernel) includes `rd.h` and exercises `rd_buf_insert`/`rd_buf_len`/etc. in KTESTs. So the kernel build of `rd_core.c`/`rd.h` must stay **Lisp-free** (no `Lobj`, no `lm.h`). Every text-property addition (the interval field, `#include "lm.h"`, the put/get/etc. functions, the GC helper, the per-char face read in `render_leaf`, the interval adjustment inside `rd_buf_insert`/delete) MUST be wrapped `#ifdef LM_BUILD ... #endif`. The lm.elf compile rule gains `-DLM_BUILD`; the kernel build does not define it, so it sees the plain gap buffer + grid exactly as today (KTESTs unaffected). `lm_core.c` and `user/lm*.c` are lm-only (not in the kernel link path for symbols they define — but they're still compiled by the wildcard into the kernel; `lm_core.c` is NOT in `src/`? it is — so it too must compile in the kernel. Verify: `src/lm_core.c` already compiles in the kernel today, so its existing code is kernel-safe; only guard NEW Lobj-in-rd code).

**Testing reality (important):** `src/rd_core.c`, `src/lm_core.c`, `user/lm*.c` compile into `lm.elf` (the userland Lisp machine), and the new Lisp-facing functions are reachable only there — so the kernel KTEST harness (`make test`) cannot call these functions. They are tested through:

...
int umain(int argc, char **argv)
{
    ...
    /* The machine's init file: /init.l on the persistent ext2 root, loaded
     ...
     * itself: (let ((fd (creat "/init.l"))) (fd-write fd ...)). (Before the
     * ext2-root cutover this lived at /disk/init.l; the root IS that
     * filesystem now, so the path is simply /init.l.) */
    if (sys_getpid() == 1) {
        lm_eval_all_str("(let ((fd (open \"/init.l\")))"
                        "  (if (< fd 0) nil"
                        "    (progn (close fd) (load \"/init.l\")"
